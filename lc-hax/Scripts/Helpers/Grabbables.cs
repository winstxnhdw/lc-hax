...
static partial class Helper {
    ...
    internal static GrabbableObject? SpawnItem(Vector3 position, Item prefab) {
        ...
        Grab.SetScrapValue((int)(random.Next(prefab.minValue + 25, prefab.maxValue + 35) *
    ...
    internal static void Detach(this GrabbableObject grabbableObject) {
        ...
        if (player.isInElevator)
            grabbableObject.transform.SetParent(round.elevatorTransform, true);
        else
            grabbableObject.transform.SetParent(round.propsContainer, true);
        ...
        if (!grabbableObject.itemProperties.syncDiscardFunction) grabbableObject.playerHeldBy = null;
    }
