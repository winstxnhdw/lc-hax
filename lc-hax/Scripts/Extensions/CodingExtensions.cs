...
static partial class Extensions {
    ...
    internal static string ReplaceWholeWord(this string original, string wordToFind, string replacement, RegexOptions regexOptions = RegexOptions.IgnoreCase) => Regex.Replace(original, wordToFind, replacement, regexOptions);

    internal static bool IsMatch(this string value, string wordToFind, RegexOptions regexOptions = RegexOptions.IgnoreCase) => Regex.IsMatch(value, wordToFind, regexOptions);
    internal static bool IsMatchWholeWord(this string value, string wordToFind, RegexOptions regexOptions = RegexOptions.IgnoreCase) => Regex.IsMatch(value, $@"\b{wordToFind}\b", regexOptions);

    internal static string RemoveWhitespace(this string str) => string.Join("", str.Split(default(string[]), StringSplitOptions.RemoveEmptyEntries));
    ...
    internal static string ConvertToString<TKey, TValue>(this IDictionary<TKey, TValue> dictionary) => "{" + string.Join(",", dictionary.Select(kv => kv.Key + "=" + kv.Value).ToArray()) + "}";
