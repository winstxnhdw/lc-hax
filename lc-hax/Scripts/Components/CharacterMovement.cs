...
class CharacterMovement : MonoBehaviour {
    ...
    void OnEnable() {
        if (this.CharacterController is not null) this.CharacterController.enabled = true;
    }

    void OnDisable() {
        if (this.CharacterController is not null) this.CharacterController.enabled = false;
    }
    ...
    void InitializeCharacterController() {
        ...
        this.CharacterController.excludeLayers = this.ignoredLayers.ToLayerMask();
        ...
        foreach (PlayerControllerB? playerControllerB in Helper.Players)
            if (playerControllerB != null) {
                ...
                foreach (Collider? collider in list) Physics.IgnoreCollision(collider, this.CharacterController);
            }

        foreach (Collider? collider2 in this.GetComponentsInChildren<Collider>())
    ...
    void Update() {
        ...
        if (this.Keyboard.spaceKey.wasPressedThisFrame) this.Jump();
    }
