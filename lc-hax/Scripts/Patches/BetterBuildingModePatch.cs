...
class BetterBuildingModePatch {
    ...
    static void BuldTipPrefix(HUDManager __instance) {
        ...
        if (!startOfRound.unlockablesList.unlockables[shipobject.unlockableID].canBeStored) {
            ...
            __instance.buildModeControlTip.text = "Reset: [Q] | Confirm: [B] | Rotate: [ R: X ¦ F: Y ¦ V: Z ] | Store: [X]";
        }
    ...
    [HarmonyPrefix]
    static bool PlayerMeetsConditionsToBuildPrefix(ref bool __result, ref bool ___CanConfirmPosition, ref PlaceableShipObject? ___placingObject) {
