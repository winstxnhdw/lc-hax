...
class FakeReloadPatch {
    ...
    class ShotGun {
        [HarmonyPatch("FindAmmoInInventory"),
         HarmonyPrefix] // This makes the shotgun look for a item slot that has a Shotgun Shell. Patch makes FindAmmoInInventory always result to 0 so it's not null.
        ...
        static bool PrefixItemInteractLeftRight(ShotgunItem __instance, ref int ___shellsLoaded) {
            // This checks if the shotgun is already loaded. Patch makes it where if it's full it unloads it so you can reload again.
        ...
        [HarmonyPatch("StopUsingGun"),
         HarmonyPrefix] // Sets InterruptDestroyItem to false if Shotgun is Dropped or Pocketed.
    ...
    class DestroyItem {
        [HarmonyPatch("DestroyItemInSlotAndSync"),
         HarmonyPrefix] // if InterruptDestroyItem it's true it immediately returns DestroyItemInSlotAndSync to prevent ShotgunItem.reloadGunAnimation to call it.
