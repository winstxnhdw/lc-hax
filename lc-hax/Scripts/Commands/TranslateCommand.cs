...
internal class TranslateCommand : ICommand {
    ...
    IEnumerator Translate(TranslateRequest request) {
    ...
    public void Execute(StringArray args) {
        if (args.Length < 3) {
            Chat.Print("Usage: translate <source> <target> <message>");
            return;
        }

        string? source = Helper.FuzzyMatch(args[0]?.ToLower(), TranslateCommand.FLORES200.Value.Keys);
        string? target = Helper.FuzzyMatch(args[1]?.ToLower(), TranslateCommand.FLORES200.Value.Keys);
        ...
        TranslateRequest request = new(
            TranslateCommand.FLORES200.Value[source],
            TranslateCommand.FLORES200.Value[target],
            string.Join(' ', args[2..])
        );

        Helper.CreateComponent<AsyncBehaviour>().Init(() => this.Translate(request));
    }
}
