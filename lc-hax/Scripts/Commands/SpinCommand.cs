...
class SpinCommand : ICommand {
    public void Execute(StringArray args) {
        ...
        if (!ulong.TryParse(args[0], out ulong duration)) Chat.Print("Invalid duration!");

        Helper.FindObjects<PlaceableShipObject>()
            .ForEach(this.SpinObject(duration));
    }

    Action<float> PlaceObjectAtRotation(PlaceableShipObject shipObject) =>
        (timeElapsed) =>
    ...
    Action<PlaceableShipObject> SpinObject(ulong duration) =>
        (shipObject) =>
